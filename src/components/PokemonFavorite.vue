<template>
    <div style="justify-content: center;padding:20%" v-if="favPokemonName == ''">
          <h3 style="color:white;">No Favorite Pokemon</h3>
    </div>
      <div class="list" v-if="pokeTypeUrl">
        <article 
          v-for="(pokemon, index) in pokemonsFiltered.filter(x => favPokemonName.includes(x.name))"
          :key="'poke'+index"
  ...
  <div class="list" v-else>
        <article 
          v-for="(pokemon, index) in pokemons.filter(x => favPokemonName.includes(x.name))"
          :key="'poke'+index"
          @click="setPokemonUrl(`https://pokeapi.co/api/v2/pokemon/${pokemon.id}`)">
          <h5 class="pokeName">{{ pokemon.name }}</h5>
            <img class="pokeImageLarge" :src="imgUrl + pokemon.id + '.png'" width="120" height="120" alt="">
